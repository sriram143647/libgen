...
async def fetch_data(session,page_url):
    global df
    file_links = []
    page = page_url.split('page=')[-1]
    topic_id = page_url.split('topicid')[1].split('&')[0]
    async with session.get(page_url,headers=header) as res:
        print(f'page: {page}')
...
async def search_by_topicid(topic_id):
    global df
    params = {
        'view_type':'simple',
        'search_field':'def',
